"""Migration: allow Pitching Index queue items.

Expands `content_queue.content_type` CHECK to include `pitching_index`.

Run:
  cd mlbops && ../mlb_env/bin/python -m api.db.migrate_pitching_index
"""
...
def _already_supports_pitching_index(conn: sqlite3.Connection) -> bool:
    row = conn.execute(
        "SELECT sql FROM sqlite_master WHERE type='table' AND name='content_queue'"
    ).fetchone()
    return bool(row and row[0] and "pitching_index" in row[0])
...
def migrate() -> None:
    ...
    try:
        if _already_supports_pitching_index(conn):
            print("content_queue already allows pitching_index.")
            return
